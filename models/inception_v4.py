...
from models.inception_model import *
class BasicConv2d(nn.Module):

    def __init__(self, input_channels, output_channels, **kwargs):
        super().__init__()
        self.conv = nn.Conv2d(input_channels, output_channels,bias=False, **kwargs)
        #self.pool = nn.MaxPool2d(2, stride=1, padding=1)
...
class Inception_Stem(nn.Module):
    ...
    def __init__(self, input_channels):
        ...
        self.branch7x7a = nn.Sequential(
            BasicConv2d(160, 64, kernel_size=1,padding=1),
            BasicConv2d(64, 64, kernel_size=(7, 1), padding=(2, 0)),
            BasicConv2d(64, 64, kernel_size=(1, 7), padding=(0, 2)),
            BasicConv2d(64, 96, kernel_size=3, padding=0)
        )

        self.branch7x7b = nn.Sequential(
            BasicConv2d(160, 64, kernel_size=1,padding=0),
            BasicConv2d(64, 96, kernel_size=3,padding=0)
        )
...
class InceptionResNetC(nn.Module):
    ...
    def __init__(self, input_channels):
        ...
        self.branch1x1 = BasicConv2d(input_channels, 192, kernel_size=1)
        self.reduction1x1 = nn.Conv2d(448, 1024, kernel_size=1)
        self.shorcut = nn.Conv2d(input_channels, 1024, kernel_size=1)
...
class InceptionResNetV2(nn.Module):
    ...
    def __init__(self, A, B, C, k=256, l=256, m=384, n=384, class_nums=5):
        super().__init__()
        self.stem = Inception_Stem(3)
        ...
        self.inception_resnet_c = self._generate_inception_module(2146, 1024, C, InceptionResNetC)

        #6x6 featuresize
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        #"""Dropout (keep 0.8)"""
        self.dropout = nn.Dropout2d(1 - 0.8)
        self.linear = nn.Linear(1024, class_nums)
    ...
    def forward(self, x):
        ...
        feat_map=torch.tensor(x)      

        x = self.avgpool(x)
        x = self.dropout(x)
        x = x.view(-1, 1024)
        feat2=torch.tensor(x)
        x = self.linear(x)

        return x,feat_map,feat2
...
def inception_resnet_v1():
    return InceptionResNetV2(5, 10, 5,192,192,256,384)

class A_NET(nn.Module):
    def __init__(self):
        super().__init__()
        #part-1 
        self.conv1=nn.Conv2d(1024, 5,kernel_size=1,bias=False)

        #part-2
        self.conv2=BasicConv2d(1024,1024,kernel_size=(3,3),padding=1)
        self.conv3=BasicConv2d(1024,1024,kernel_size=(3,3),padding=1)
        self.conv4=nn.Conv2d(1024,5,kernel_size=1,padding=0)
    ...
    def forward(self, x):
        ...
        x2=x2.view(-1,5,14,14)
        x=x1*x2
        a_map=torch.tensor(x)
        x=x.view(-1,5,14*14)
        x=x.sum(2)
        return x,a_map
...
class zoom_in_net(nn.Module):

    def __init__(self,class_nums=5):
        super().__init__()
        self.fc= nn.Linear(2048, class_nums)

        # batch*1024
    def forward(self,ya,ym,featm,featc):
        feat=[featm,featc]
...
class zoom_in_net(nn.Module):
    ...
    def __init__(self):
        super().__init__(mnet_model_path=None,anet_model_path=None,cnet_model=None)
        
        self.m_net=inception_resnet_v1()
        if mnet_model_path!= None:
            self.m_net.load_state_dict(torch.load(mnet_model_path))
        # batchsize*5 fenlei  batch*1024*14*14 map  batch*1024 fea
        self.a_net=A_NET()
        if anet_model_path!=None:
            self.a_net.load_state_dict(torch.load(anet_model_path))
        # batch*5 batch*5*14*14
        self.c_net=inception_model.inception_v3()
        if cnet_model_path!=None:
            self.c_net.load_state_dict(torch.load(cnet_model_path))
        self.fc= nn.Linear(2048, class_nums)

        # batch*1024
    def forward(self,x,images):
        # x:batch*3*492*492 images:batch*3*yuan0*yuan1
        ym,feat_map,featm=self,m_net(x)
        ya,a_map=self.a_net(feat_map)
        num=len(images)
        input_data=[]
        for idx in range(num):
            height=images[idx].shape[0]
            width=images[idx].shape[1]
            img=images[idx]
            a_map_np=np.array(a_map[idx])
            a_map_np=a_map_np.max(0)
            a_map_np=cv2.resize(a_map_np,(height,width),interpolation=cv2.INTER_AREA)
            crop_img=np.zeros((4,200,200))
            a_map_np=np.pad(a_map_np,((120,120),(120,120)),'constant',constant_values=(0,0))
            img=np.pad(img,((120,120),(120,120)),'constant',constant_values=(0,0))
            for i in range(4):
                m,n=a_map_np.shape
                index= int(a_map_np.argmax())
                x = int(index / n)
                y = index % n
                left=x-100
                right=x+100-1
                bottom=y-100
                up=y+100-1
                crop_img[i]=img[left:right,bottom:up].copy()
                a_map_np[left:right,bottom:up]=0
            input_data.append(crop_img)
