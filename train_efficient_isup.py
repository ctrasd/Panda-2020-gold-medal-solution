...
parser.add_argument('--size', default='256', choices=['154','140','256','192'])
...
class panda_dataset_random(Dataset):
    ...
    def __getitem__(self, index):
        ...
        cv2.imwrite('./save_imgs/'+self.size+'_fanfan'+str(index%100)+'.png',img)
...
if __name__ == '__main__':
    ...
    writer=SummaryWriter(comment=args.size+'_'+args.model+'_00001_adam_bce_fold_augb_'+args.fold+'_isup_ji')
    ...
    if args.model=='dense':
        ...
        '''
        net=isupnet(num_classes=6,base_model=args.model)
    if use_gpu:
        net= nn.DataParallel(net).cuda()
    net.load_state_dict(torch.load('./save_models/model_256_efficientnet-b0_0.0003_adam_4_bce_newest_isup.pkl'))
    #criterion = nn.CrossEntropyLoss()
    #criterion = nn.SmoothL1Loss()
    criterion=nn.BCEWithLogitsLoss()
    #criterion = nn.MSELoss()
    optimizer = optim.Adam(net.parameters(),lr=args.lr,betas=(0.9,0.999), eps=1e-08, weight_decay =0.0)
    #scheduler=torch.optim.lr_scheduler.MultiStepLR(optimizer,[15,50], gamma=0.1, last_epoch=-1)
    ...
    scheduler =torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min', factor=0.8, patience=3, verbose=True, eps=1e-6,min_lr=1e-5)
    ...
    for epoch in range(start_epoch,args.max_epoch):
        ...
        torch.save(state_dict, './save_models/model_'+args.size+'_'+args.model+'_'+str(args.lr)+'_adam_'+args.fold+'_bce_newest_isup_ji.pkl')
        if kappa>max_correct:
            max_correct=kappa
            print('saved kappa:',max_correct)
            state_dict = net.state_dict()
            torch.save(state_dict, './save_models/model_'+args.size+'_'+args.model+'_'+str(args.lr)+'_adam_'+args.fold+'_bce_maxest_isup_ji.pkl')
        if total_loss/total<max_loss:
            max_loss=total_loss/total
            print('saved min_loss:',max_loss)
            state_dict = net.state_dict()
            torch.save(state_dict, './save_models/model_'+args.size+'_'+args.model+'_'+str(args.lr)+'_adam_'+args.fold+'_bce_min_loss_isup_ji.pkl')
        if kappa_kar>max_kar_kappa:
            max_kar_kappa=kappa_kar
            print('saved max_kar:',max_kar_kappa)
            state_dict = net.state_dict()
            torch.save(state_dict, './save_models/model_'+args.size+'_'+args.model+'_'+str(args.lr)+'_adam_'+args.fold+'_bce_karmax_isup_ji.pkl')
